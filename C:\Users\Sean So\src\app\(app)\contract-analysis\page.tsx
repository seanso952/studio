...
import { analyzeContractTime, type ContractTimeAnalysisOutput } from '@/ai/flows/contract-time-analysis';
...
export default function ContractAnalysisPage() {
  ...
  const onSubmit: SubmitHandler<ContractAnalysisFormValues> = async (data) => {
    ...
    try {
      ...
      console.error("Error analyzing contract time:", err);
    ...
    <div className="space-y-6">
      ...
      {analysisResult && (
        <Card className="shadow-lg mt-6">
          ...
          <CardContent className="space-y-4">
            <div>
              <FormLabel className="text-sm font-medium text-muted-foreground">Time Spent</FormLabel>
              <p className="text-lg font-semibold">{analysisResult.timeSpent}</p>
            </div>
            <div>
              <FormLabel className="text-sm font-medium text-muted-foreground">Time Remaining</FormLabel>
            ...
            {analysisResult.nearExpiryAlert && (
             ...
             {!analysisResult.nearExpiryAlert && (
                <Alert variant="default" className="bg-green-50 border-green-300 text-green-700 dark:bg-green-900/30 dark:border-green-700 dark:text-green-300">
                    ...
                    <AlertDescription>Contract is not near expiry.</AlertDescription>
                </Alert>
            )}
          </CardContent>
        </Card>
